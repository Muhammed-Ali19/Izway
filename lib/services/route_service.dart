...
class RouteService {
  ...
  Future<List<RouteInfo>> calculateRoutes(LatLng start, LatLng destination) async {
    ...
    // Si nous n'avons gardé qu'une seule option (car elles étaient toutes identiques), 
    // l'appeler "Autoroute" peut porter à confusion s'il n'y en a pas.
    // On la renomme de manière générique.
    if (allRoutes.length == 1) {
      allRoutes[0] = RouteInfo(
        points: allRoutes[0].points,
        duration: allRoutes[0].duration,
        distance: allRoutes[0].distance,
        label: "Le plus rapide",
        speedLimits: allRoutes[0].speedLimits,
        steps: allRoutes[0].steps,
        borderCrossings: allRoutes[0].borderCrossings,
        countries: allRoutes[0].countries,
      );
    }
  ...
  bool _isDifferentRoute(RouteInfo route, List<RouteInfo> existingRoutes) {
    for (var existing in existingRoutes) {
      // On compare la durée et la distance pour éviter les vrais doublons géométriques
      // On considère que c'est le même trajet si la différence de temps est < 2 minutes (120s)
      // et la différence de distance < 2 km (2000m).
      final double timeDiff = (route.duration - existing.duration).abs();
      final double distDiff = (route.distance - existing.distance).abs();
      
      if (timeDiff < 120 && distDiff < 2000) {
        // C'est géographiquement la même route, on ignore le nouveau doublon
        return false;
      }
    }
    return true;
